...
#include "cudalib/mel-compute.h"

CudaMelBanks::CudaMelBanks(const CudaMelBanksOptions &opts,
...
BaseFloat CudaMelBanks::VtlnWarpFreq(BaseFloat vtln_low_cutoff,  // upper+lower frequency cutoffs for VTLN.
...
BaseFloat CudaMelBanks::VtlnWarpMelFreq(BaseFloat vtln_low_cutoff,  // upper+lower frequency cutoffs for VTLN.
                                    BaseFloat vtln_high_cutoff,
                                    BaseFloat low_freq,  // upper+lower frequency cutoffs in mel computation
                                    BaseFloat high_freq,
                                    BaseFloat vtln_warp_factor,
...
void CudaMelBanks::Compute(const CuVectorBase<BaseFloat> &power_spectrum,
                       CuVector<BaseFloat> *mel_energies_out) const
{
  ...
  for (int32 i = 0; i < num_bins; i++)
  {
    ...
    const CuVector<BaseFloat> &v(bins_[i].second);
...
BaseFloat CudaDurbin(int n, const BaseFloat *pAC, BaseFloat *pLP, BaseFloat *pTmp)
{
...
void CudaLpc2Cepstrum(int n, const BaseFloat *pLPC, BaseFloat *pCepst)
{
  for (int32 i = 0; i < n; i++)
  {
    double sum = 0.0;
