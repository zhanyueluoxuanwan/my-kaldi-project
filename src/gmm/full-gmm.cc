...
int32 FullGmm::ComputeGconsts() {
  ...
  valid_gconsts_ = true;
  KALDI_LOG << "compute gconsts done";
  return num_bad;
}
...
void FullGmm::LogLikelihoodsPreselect(const VectorBase<BaseFloat> &data,
                                      ...
                                      Vector<BaseFloat> *loglikes) const {
//  KALDI_LOG << "gconsts dim:" << gconsts_.Dim();
//  KALDI_LOG << "weights dim:" << weights_.Dim();
//  KALDI_LOG << "inv_covars size:" << inv_covars_.size() << "\tinv_covars rows:" << inv_covars_[0].NumRows() << "\tinv_covars cols:" << inv_covars_[0].NumCols();
//  KALDI_LOG << "\tmeans_inv_covars rows:" << means_invcovars_.NumRows() << "\tmeans_inv_covars cols:" << means_invcovars_.NumCols();
  int32 dim = Dim();
  KALDI_ASSERT(dim == data.Dim());
